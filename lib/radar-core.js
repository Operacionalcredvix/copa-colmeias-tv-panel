...
const VERSION = 'RADAR_VERCEL_MVP_4';
  ...
  for (let r = 0; r < values.length; r++) {
    ...
    for (let i = r + 1; i < values.length; i++) {
      ...
      byName[storeKey] = {
        ...
        responsible: rowResponsible || currentResponsible || previous.responsible || '',
...
function buildResponsiblePerformance(coordinators, todayRows, storeGoals) {
  ...
  Object.entries(today).forEach(([name, row]) => {
    ...
    output.push({
      ...
      diagnosis: 'Coordenadora com produção transacional, mas ausente no consolidado do PAINEL COMERCIAL.'
    });
  });
