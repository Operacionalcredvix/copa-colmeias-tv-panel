...
const VERSION = 'RADAR_VERCEL_MVP_2';
...
async function batchGet(spreadsheetId, ranges) {
  ...
  (json.valueRanges || []).forEach((vr, index) => {
    ...
    // O Google pode devolver um range menor que o solicitado.
    // Exemplo: pedimos A1:Z5000 e ele devolve A1:K4030.
    // Por isso salvamos pelos dois nomes: range solicitado e range devolvido.
...
function readCredvixMap(values) {
  ...
  const fallback = {
    ...
    DENYSSANE: 'Credvix Vila Velha',
    'DANIELE CASTELAR': 'Credvix Vila Velha',
    'DANIELE SANTOS': 'Credvix Vila Velha',
    'HELEN MENDES': 'Credvix Vila Velha'
  };
...
function resolveStore(storeRaw, storeMap, credvixMap, consultant) {
  ...
  if (code === '55634') {
    ...
    if (!target) {
      return {
        skip: true,
        name: 'CREDVIX Digital 1',
        responsible: 'SEM COORDENADORA'
      };
    }
    ...
    return item
      ...
      : { name: cleanStoreName(target), responsible: 'SEM COORDENADORA' };
  }
...
function readStoreGoals(projecaoValues, metasValues) {
  ...
  readStoreGoalBlocks(metasValues, byName, 'METAS JULHO');
...
function buildResponsiblePerformance(coordinators, todayRows) {
  ...
  return coordinators
    ...
    .sort((a, b) => b.productionToday - a.productionToday);
}
