...
export async function getRadarPayload({ noCache = false } = {}) {
  ...
  const latestLoadAt = latestLoadLabel(productionMonth.rows) || nowHour();
  const panelUpdatedAt = nowHour();
...
function buildResponsiblePerformance(coordinators, todayRows, storeGoals) {
  ...
  coordinators.forEach((coordinator) => {
    ...
    const dailyGap = dailyGoal ? Math.max(0, dailyGoal - row.production) : null;
  ...
  Object.entries(today).forEach(([name, row]) => {
    ...
    const dailyGap = responsibleGoals.dailyGoal
      ? Math.max(0, responsibleGoals.dailyGoal - row.production)
      : null;
...
export async function buildDeepSeekReading(payload) {
  ...
  const prompt = [
    ...
    'Responda somente JSON válido. Não use markdown.',
    'Se algum dado estiver ausente, sinalize como dado ausente sem estimar valores.',
    'Limites: headline com até 80 caracteres, executiveSummary com até 160 caracteres, actions com no máximo 3 itens, risks com no máximo 3 itens, zeroStoresCritical com no máximo 6 itens e questions com no máximo 3 itens.',
    '',
    'Formato obrigatório:',
  ...
  const response = await fetch(process.env.DEEPSEEK_API_URL || 'https://api.deepseek.com/chat/completions', {
    ...
    body: JSON.stringify({
      ...
      messages: [
        {
          role: 'system',
          content: 'Responda somente JSON válido. Não use markdown. Use português do Brasil, com foco executivo e acionável.'
        },
        {
          role: 'user',
          content: prompt
  ...
  const structured = parseDeepSeekStructured(text);
...
function parseDeepSeekStructured(text) {
  ...
  try {
    return normalizeDeepSeekStructured(JSON.parse(source));
  } catch {
    return null;
  }
}
...
function normalizeDeepSeekStructured(value) {
  if (!value || typeof value !== 'object' || Array.isArray(value)) {
    return null;
  }

  return {
    headline: shortText(value.headline, 90),
    executiveSummary: shortText(value.executiveSummary, 180),
    priorityCoordinator: shortText(value.priorityCoordinator, 80),
    actions: normalizeAiList(value.actions, 3, normalizeAiAction),
    risks: normalizeAiList(value.risks, 3, normalizeAiRisk),
    zeroStoresCritical: normalizeAiList(value.zeroStoresCritical, 6, normalizeAiZeroStore),
    questions: normalizeAiList(value.questions, 3, (item) => shortText(item, 130)).filter(Boolean)
  };
}

function normalizeAiList(value, limit, mapper) {
  return Array.isArray(value)
    ? value.map(mapper).filter(Boolean).slice(0, limit)
    : [];
}

function normalizeAiAction(item) {
  if (typeof item === 'string') {
    return {
      title: shortText(item, 90),
      detail: '',
      responsible: '',
      severity: 'attention'
    };
  ...
  return {
    title: shortText(item.title || item.action || item.name, 90),
    detail: shortText(item.detail || item.description, 150),
    responsible: shortText(item.responsible || item.coordinator || item.priorityCoordinator, 80),
    severity: normalizeSeverity(item.severity)
  };
}

function normalizeAiRisk(item) {
  if (typeof item === 'string') {
    return {
      title: shortText(item, 90),
      detail: '',
  ...
  return {
    title: shortText(item.title || item.risk || item.name, 90),
    detail: shortText(item.detail || item.description, 150),
    severity: normalizeSeverity(item.severity)
  };
}

function normalizeAiZeroStore(item) {
  if (typeof item === 'string') {
    return {
      name: shortText(item, 90),
      responsible: '',
      impact: ''
    };
  }
  ...
  return {
    name: shortText(item.name || item.store, 90),
    responsible: shortText(item.responsible || item.coordinator, 80),
    impact: shortText(item.impact || item.detail, 130)
  };
}

function normalizeSeverity(value) {
  const severity = String(value || '').toLowerCase();
  return ['critical', 'attention', 'normal'].includes(severity) ? severity : 'attention';
}
...
function buildAiInput(payload) {
  return {
    ...
    latestLoadAt: payload.latestLoadAt,
    panelUpdatedAt: payload.panelUpdatedAt,
...
function latestLoadLabel(rows) {
  const loads = rows
    .map((row) => parseLoadStamp(first(row, ['CARGA_EM', 'CARGA EM', 'ULTIMA CARGA'])))
    .filter(Boolean)
    .sort((a, b) => b.sortKey - a.sortKey);

  return loads[0]?.label || '';
}

function parseLoadStamp(value) {
  if (!value) {
    return null;
  }
  ...
  if (typeof value === 'number' || value instanceof Date) {
    ...
    return date
      ? { sortKey: date.getTime(), label: formatHour(date) }
      : null;
  }
  ...
  if (match) {
    ...
    return {
      ...
      label: hourMinuteLabel(hour, minute)
    };
  }
...
function hourMinuteLabel(hour, minute) {
