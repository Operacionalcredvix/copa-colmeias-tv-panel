...
const VERSION = 'RADAR_VERCEL_MVP_6';
...
export async function getRadarPayload({ noCache = false } = {}) {
  ...
  const dailyOperational = readDailyOperational(
    management[managementRanges[3]] || [],
    storeMap
  );

  const summary = summarizeRows(todayRows);
  summary.sold = dailyOperational.summary.soldToday || managementSummary.todaySold || 0;
  summary.paid = dailyOperational.summary.paidToday ?? summary.production;
  summary.conversionPending = Math.max(0, summary.sold - summary.paid);

  const yesterdaySummary = summarizeRows(yesterdayRows);
  const sevenAvg = averageByDate(sevenRows);
  const goal = buildGoal(summary, managementSummary, dailyOperational.summary);
  const operationalStores = dailyOperational.stores;
  const zeroStores = operationalStores.length
    ? buildOperationalZeroStores(operationalStores)
    : buildZeroStores(storeMap, todayRows, storeGoals);
  const topStoresByProduction = operationalStores.length
    ? rankOperationalStores(operationalStores, false).slice(0, 10)
    : groupStores(todayRows, storeGoals, false).slice(0, 10);
  const topStoresByGoal = operationalStores.length
    ? rankOperationalStores(operationalStores, true).slice(0, 10)
    : groupStores(todayRows, storeGoals, true).slice(0, 10);
  const responsiblePerformance = mergeOperationalResponsible(
    buildResponsiblePerformance(coordinators, todayRows, storeGoals),
    dailyOperational
  );
  const paceContext = buildPaceContext(rawRows, today, summary, goal);
  const missingData = missing({
    managementSummary: { ...managementSummary, dailyGoal: goal.dailyGoal },
    storeGoals,
    paceContext,
    zeroStores,
    coordinators,
  ...
  const payload = {
    ...
    version: VERSION,
    generatedAt: nowDateTime(),
    updatedAt: managementSummary.updatedAt || latestLoadLabel(productionMonth.rows) || nowHour(),
    date: formatDateBr(today),
    responsibleLabel: 'COORDENADORA',

    summary: {
      contractsToday: summary.contracts,
      productionToday: round2(summary.paid),
      productionTodayFormatted: money(summary.paid),
      paidToday: round2(summary.paid),
      paidTodayFormatted: money(summary.paid),
      soldToday: round2(summary.sold),
      soldTodayFormatted: money(summary.sold),
      conversionPending: round2(summary.conversionPending),
      conversionPendingFormatted: money(summary.conversionPending),
      averageTicket: round2(summary.averageTicket),
      averageTicketFormatted: money(summary.averageTicket),
      activeStores: operationalStores.length
        ? operationalStores.filter((store) => store.soldToday > 0).length
        : summary.activeStores,
      paidActiveStores: operationalStores.length
        ? operationalStores.filter((store) => store.paidToday > 0).length
        : summary.activeStores,
      totalStores: operationalStores.length || storeMap.stores.length,
      zeroStores: zeroStores.length,
      zeroPaidStores: operationalStores.filter((store) => store.paidToday <= 0).length
    },

    goal,

    rhythm: buildOperationalRhythm(summary, goal, paceContext),
    ...
    counts: {
      ...
      operationalStores: operationalStores.length,
      soldActiveStores: operationalStores.filter((store) => store.soldToday > 0).length,
      paidActiveStores: operationalStores.filter((store) => store.paidToday > 0).length
    },
...
function readDailyOperational(values, storeMap) {
  const stores = [];
  const coordinators = [];
  const summary = {};
  ...
  for (let r = 0; r < values.length; r++) {
    ...
    if (headers[0] === 'COORDENADORA' && headers.includes('VENDIDO HOJE')) {
      for (let i = r + 1; i < values.length; i++) {
        ...
        const item = {
          ...
          monthPercent: valueByHeader(headers, dataRow, ['% PROJETADO', '%']),
    ...
    if (headers[0] === 'LOJA' && headers.includes('VENDIDO HOJE')) {
      for (let i = r + 1; i < values.length; i++) {
        const dataRow = values[i] || [];
        const rawStore = String(dataRow[0] || '').trim();
        const rawNorm = norm(rawStore);

        if (!rawStore || rawNorm.indexOf('COORDENACAO') === 0) {
          break;
        }

        if (rawNorm.indexOf('TOTAL') === 0) {
          break;
        }

        const resolved = resolveStore(rawStore, storeMap, null, '');
        ...
        stores.push({
          name: resolved.name,
          responsible: currentResponsible || resolved.responsible,
          soldToday,
          ...
          dailyGoal,
          goalDaily: dailyGoal,
          goalDailyFormatted: dailyGoal ? money(dailyGoal) : 'DADO AUSENTE',
          soldPercent: dailyGoal ? Math.round((soldToday / dailyGoal) * 100) : 'DADO AUSENTE',
          paidPercent: dailyGoal ? Math.round((paidToday / dailyGoal) * 100) : 'DADO AUSENTE',
          monthGoal,
          monthGoalFormatted: monthGoal ? money(monthGoal) : 'DADO AUSENTE',
          monthRealized,
          monthRealizedFormatted: money(monthRealized),
          monthPercent: valueByHeader(headers, dataRow, ['% ATINGIDO', '%']),
          status: textByHeader(headers, dataRow, ['STATUS']),
          insight: textByHeader(headers, dataRow, ['INSIGHT'])
        });
      }
    }
  }
...
function rankOperationalStores(stores, byGoal) {
  const out = stores.map((store) => ({
    ...store,
    contracts: 0,
    production: store.soldToday,
    productionFormatted: store.soldTodayFormatted,
    goalPercent: store.soldPercent,
    goalGapFormatted: store.dailyGoal
      ? money(Math.max(0, store.dailyGoal - store.soldToday))
      : 'DADO AUSENTE'
  }));

  out.sort((a, b) => {
    if (byGoal) {
      const aPercent = typeof a.soldPercent === 'number' ? a.soldPercent : -1;
      const bPercent = typeof b.soldPercent === 'number' ? b.soldPercent : -1;
      return bPercent - aPercent || b.soldToday - a.soldToday;
    }

    return b.soldToday - a.soldToday || b.monthRealized - a.monthRealized;
  });

  return out.map((item, index) => ({ ...item, position: index + 1 }));
}

function buildOperationalZeroStores(stores) {
  return stores
    ...
    .map((store) => ({
      ...
      dailyGoalFormatted: store.goalDailyFormatted,
      monthGoalFormatted: store.monthGoalFormatted
    }))
    .sort((a, b) => {
...
function mergeOperationalResponsible(baseRows, operational) {
  if (!operational.coordinators.length) {
    return baseRows;
  }

  const baseByName = Object.fromEntries(
    baseRows.map((row) => [norm(row.name), row])
  );
  const seen = new Set();

  const merged = operational.coordinators.map((item) => {
    const key = norm(item.name);
    const base = baseByName[key] || {};
    seen.add(key);
    const pending = Math.max(0, item.soldToday - item.paidToday);
    const soldPercent = item.dailyGoal
      ? Math.round((item.soldToday / item.dailyGoal) * 100)
      : 'DADO AUSENTE';
    const paidPercent = item.dailyGoal
      ? Math.round((item.paidToday / item.dailyGoal) * 100)
      : 'DADO AUSENTE';

    return {
      ...base,
      name: item.name,
      contractsToday: base.contractsToday || 0,
      ...
      dailyGoalFormatted: item.dailyGoal ? money(item.dailyGoal) : 'DADO AUSENTE',
      dailyGapFormatted: item.dailyGoal
        ? money(Math.max(0, item.dailyGoal - item.paidToday))
        : 'DADO AUSENTE',
      soldPercent,
      ...
      risk: item.paidToday <= 0 && item.soldToday > 0
        ? 'Conversão pendente'
        : item.status || base.risk || 'Monitorar',
      priority: item.soldToday > item.paidToday
        ? 'Converter vendido em pago'
        : base.priority || 'Acompanhar operação',
      diagnosis: pending > 0
        ? `${money(pending)} vendidos aguardando pagamento.`
        : base.diagnosis || 'Acompanhar entrega da diária.'
    };
  });

  baseRows.forEach((row) => {
    if (!seen.has(norm(row.name))) {
      merged.push(row);
    }
  });

  return merged.sort((a, b) => {
    return Number(b.conversionPending || 0) - Number(a.conversionPending || 0) ||
      Number(b.soldToday || 0) - Number(a.soldToday || 0);
  });
}
...
function readManagementSummary(values) {
  ...
  const updateLabel = values
    ...
    .find((value) => /^\d{2}\/\d{2}\/\d{4}\s+\d{2}:\d{2}$/.test(value));

  if (updateLabel) {
    out.updatedAt = updateLabel;
  }
...
function buildGoal(summary, management, operational = {}) {
  const dailyGoal = operational.dailyGoal || management.dailyGoal || 0;
  const soldToday = operational.soldToday || summary.sold || management.todaySold || 0;
  const paidToday = operational.paidToday ?? summary.paid ?? summary.production;
  const dailyGap = dailyGoal ? Math.max(0, dailyGoal - paidToday) : null;
  const soldGap = dailyGoal ? Math.max(0, dailyGoal - soldToday) : null;
  const monthGoal = operational.monthGoal || management.monthGoal || 0;
  const monthRealized = operational.monthRealized || management.monthRealized || 0;
  ...
  return {
    ...
    dailyPercent: dailyGoal ? Math.round((paidToday / dailyGoal) * 100) : 'DADO AUSENTE',
    soldGap,
    soldGapFormatted: dailyGoal ? money(soldGap) : 'DADO AUSENTE',
    soldPercent: dailyGoal ? Math.round((soldToday / dailyGoal) * 100) : 'DADO AUSENTE',
    conversionPending: Math.max(0, soldToday - paidToday),
    conversionPendingFormatted: money(Math.max(0, soldToday - paidToday)),
    ...
    monthGoalFormatted: monthGoal ? money(monthGoal) : 'DADO AUSENTE',
    monthRealized: monthRealized || null,
    monthRealizedFormatted: monthRealized ? money(monthRealized) : 'DADO AUSENTE',
    monthPercent: monthGoal ? Math.round((monthRealized / monthGoal) * 100) : 'DADO AUSENTE',
    monthGapFormatted: monthGoal ? money(Math.max(0, monthGoal - monthRealized)) : 'DADO AUSENTE',
...
function buildOperationalRhythm(summary, goal, paceContext) {
  if (summary.conversionPending > 0 && summary.paid <= 0) {
    return {
      label: 'CONVERSÃO É A PRIORIDADE',
      tone: 'attention',
      percent: typeof goal.soldPercent === 'number' ? goal.soldPercent : 0,
      description: `${money(summary.sold)} vendidos aguardam pagamento • ${goal.soldGapFormatted} ainda faltam em vendas para a diária`
    };
  }

  if (goal.dailyGoal) {
    return {
      label: summary.paid >= goal.dailyGoal ? 'DIÁRIA ENTREGUE' : 'ACELERAR A ENTREGA',
      tone: summary.paid >= goal.dailyGoal ? 'positive' : 'negative',
      percent: typeof goal.dailyPercent === 'number' ? goal.dailyPercent : 0,
      description: `${money(summary.paid)} pagos de ${goal.dailyGoalFormatted} • ${goal.dailyGapFormatted} para a diária`
    };
  }
...
function deterministicReading(summary, goal, pace, zeroStores, responsible) {
  ...
  return {
    ...
    generatedAt: nowHour(),
    text: `${gap} ${ritmo} ${zeroStores.length} lojas sem venda exigem acionamento nominal.`,
...
function buildTicker(summary, goal, zeroStores, topStores) {
  return [
    `Vendido hoje: ${money(summary.sold)}`,
    `Pago hoje: ${money(summary.paid)}`,
    `Aguardando conversão: ${money(summary.conversionPending)}`,
    `Diária: ${goal.dailyGoalFormatted}`,
    `Sem venda hoje: ${zeroStores.length}`,
    topStores?.[0]
      ? `Líder em venda: ${topStores[0].name} — ${topStores[0].productionFormatted}`
      : 'Sem venda por loja'
  ];
}
